...
const Loginpage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("https://inventory-app-backend-production-75de.up.railway.app/api/admins", formData);
  ...
  <div className='bg-white/80 backdrop-blur-sm w-[35%] max-md:w-[90%] h-[30rem] rounded-lg shadow-xl ring-1 ring-gray-200/50 p-8'>
          ...
          <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
            <form onSubmit={handleSubmit} action={(handleSubmit) => { "/dashboard" }} className="space-y-6">
              ...
              <div>
                <button
                  type="submit"
                  className="flex w-full justify-center rounded-md bg-gray-800 px-3.5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-gray-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-gray-600 transition-colors duration-200"
                >
                  Sign in
                </button>
              </div>
